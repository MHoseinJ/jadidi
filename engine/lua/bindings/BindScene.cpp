...
void LuaBindings::bindScene(sol::state& lua) {
    ...
    gameobject.set_function("find", sol::overload(
    ...
    gameobject.set_function("deleteById",
                            sol::overload([](uint64_t id) { SceneManager::getInstance().deleteObjectById(id); },
                                          [](GameObjectHandle handle) {
                                              if (GameObject* go = handle.resolve())
                                                  SceneManager::getInstance().deleteObjectById(go->id);
                                          }));

    gameobject.set_function("deleteByName",
                            [](const std::string& name) { SceneManager::getInstance().deleteAllObjectsByName(name); });

    gameobject.set_function("deleteByTag",
                            [](const std::string& tag) { SceneManager::getInstance().deleteAllObjectsByTag(tag); });
    ...
    // Camera
    lua.new_usertype<Camera>("Camera", "transform", &Camera::transform, "zoom", &Camera::zoom);
    lua["Camera"] = &camera;
